...
namespace QrPaySystem.PageHB
{
    ...
    public class QrTool_HB
    {
        ...
        public static string genHbSkPic(string HB_QR, string Tittle, string Id, string HbSkDir)
        {
            ...
            if (File.Exists(AppDomain.CurrentDomain.BaseDirectory + HbSkDir + picName)) return picName;
            else return "";
        }
        ...
        /// 生成红包二维码
        /// </summary>
        /// <param name="HB_QR">红包二维码地址</param>
        /// <param name="tittle">商家名称</param>
        /// <returns></returns>
        public static Bitmap ToHbQr(string HB_QR, string tittle)
        {
            Bitmap QR = ToQR(HB_QR);    // 生成二维码
            Bitmap BG = getHBSK_BG();   // 获取红包收款码背景图

            Bitmap HbQr = CombineHbSK(BG, QR, tittle);
        ...
        /// <param name="tittle">商家名称</param>
        /// <returns></returns>
        private static Bitmap CombineHbSK(Bitmap BG, Bitmap QR, string tittle)
        {
            //创建图像
            ...
            DrawString(g, drawFont, Color.White, 256, 804, tittle, 0);
        ...
        private static Bitmap HBSK_BG = null;
        /// <summary>
        /// 获取红包收款码背景图
        /// </summary>
        /// <returns></returns>
        private static Bitmap getHBSK_BG()
        {
            if (HBSK_BG == null)
            {
                // 载入收款红包码背景图
                string BG_path = AppDomain.CurrentDomain.BaseDirectory + "tools\\HB_pic\\eaxmple.png";
                Image pic = Bitmap.FromFile(BG_path);

                HBSK_BG = ToBitmap(pic);
        ...
        /// 保存图像返回文件名
        /// </summary>
        public static String Save(Bitmap pic, string SubDir = "QR_HB\\", string fileName = "")
        {
